...
def fitness_value(sparrow, w1, w2, w3, sensing_radius, deployment_area, random_targets):
    ...
    # --- START OF NEW D_VAR CALCULATION (GRID-BASED) ---
    grid_size = 5 # Create a 5x5 virtual grid
    cell_size = deployment_area / grid_size

    # Use numpy's histogram2d to count sensors in each grid cell
    counts, _, _ = np.histogram2d(
        sparrow[:, 0], # All sensor x-coordinates
        sparrow[:, 1], # All sensor y-coordinates
        # Define the bins for the 5x5 grid
    ...
    # D_var is the variance of the counts in each cell
    # Clumped = [20, 0, 0...] = HIGH variance (Bad)
    # Spread = [1, 1, 0, 1...] = LOW variance (Good)
    dvar = np.var(counts)
    # --- END OF NEW D_VAR CALCULATION ---

    # Normalize the variance (this is an approximation)
    # A perfect uniform distribution (20 nodes in 25 cells) has some variance,
    # but a fully clumped one (20 in 1 cell) is much higher.
    # We can normalize by the max possible variance (all nodes in one cell)
    # This isn't perfect, but it scales the value.
    max_dvar_approx = np.var([len(sparrow)] + [0]*(grid_size**2 - 1))
    if max_dvar_approx == 0:
        normalized_dvar = 0
    else:
        normalized_dvar = dvar / max_dvar_approx

    # Fitness function will try to MINIMIZE normalized_dvar,
    # which now correctly means spreading the nodes out.
    fitness = (w1 * coverage) - (w2 * normalized_dvar) - (w3 * energy)
    return fitness
